...
function CalendarView({events}) {
  ...
  const startOfWeekDate = startOfWeek(selectedDate, { weekStartsOn: 0 });// Empieza desde domingo
  ...
  const calculateEventStyle = (start, end) => {
    ...
    const [startHour, startMinute] = start.split(":").map(Number);
    const topOffset = ((startHour * 60 + startMinute) / 60) * 4 * 16; // Empieza desde el minuto correcto
    const height = (duration / 60) * 4 * 16; // Altura basada en duración
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="calendar-header grid grid-cols-8 border-t border-border">
          ...
          {[...Array(7)].map((_, i) => {
            ...
            return (
              <div key={i} className="p-2 text-center font-semibold border-l border-border">
                {format(day, "EEE dd")}
              </div>
            );
          })}
